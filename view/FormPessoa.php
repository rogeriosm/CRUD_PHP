...
if (empty($_SESSION['login']) && $_GET['form'] != 1) {
...
require_once $_SERVER['DOCUMENT_ROOT'] . '/crud_php/controle/ListarFormControle.php';
//criando um novo controlador
$controleListar = new ListarFormControle();

//usa a mesma pagina para editar ou cadastrar um usuario
if (!empty($_SESSION['login']) && $_GET['form'] == 3) {
    //se for para editar muda o caminho do post
    $page = "/crud_php/controle/EditarPessoaControle.php";
    //busca um usuario
    $usuarioEdite = $controleListar->buscaUsuario($_GET['idPessoa']);
} else {
    $page = "/crud_php/controle/SalvarPessoaController.php";
}
...
//setando as nulo se nao tiver valor
